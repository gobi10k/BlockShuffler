...
### NEXT UP
REMAINING BEFORE DELIVERY (updated 2026-07-15b — Carter corrections 2.7/4.6/13.4 LANDED, acceptance re-baselined c6c79a6): (1) 13.4 manual zoom re-verify on the FRESH Release build (plan cell PENDING); (2) Windows slice — 12.2 spot-run, Section C parity, Windows-Release column, MSVC harness, per WINDOWS_PACKAGING.md; (3) remaining BEYOND-SPEC decisions pending Carter; (4) optional :340 clamp (TODO 7 — root-caused, one-line fix parked, needs T27/T28 re-verify).
...
- REMAINING: 13.4 manual zoom check on the fresh Release build; Windows slice (12.2 + Section C + MSVC harness); two other BEYOND-SPEC decisions still pending Carter.
