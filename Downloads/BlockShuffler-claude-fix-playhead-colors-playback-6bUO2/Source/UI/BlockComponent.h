...
private:
    ...
    bool                     isShiftDrag  = false;  ///< Shift was held when this drag started
...
public:
    ...
    /// @param isShiftDrag Whether Shift was held when the drag started.
    std::function<void(BlockComponent*, juce::Point<int>, bool)> onDragMoved;
    ...
    /// @param isShiftDrag Whether Shift was held when the drag started.
    std::function<void(BlockComponent*, juce::Point<int>, bool)> onDragEnded;

    /// Fired once at the start of a Shift+drag for a stacked block.
    /// Lets the strip record start positions of all sibling stack tiles.
    std::function<void(int stackGroup)> onBeginStackDrag;

    /// Fired each frame during a Shift+drag with the current pixel delta from drag-start.
    /// Lets the strip reposition sibling stack tiles to match.
    std::function<void(int stackGroup, BlockComponent* dragged, juce::Point<int> delta)> onMoveStackComponents;
