...
void MoogFilter::updateCoefficients(float modHz) {
    ...
    // Tuning correction: as resonance increases, the 1-sample delay in the feedback
    // loop pushes the peak frequency up slightly. We compensate by lowering fc.
    fc *= (1.0f - 0.08f * resonance_);
...
float MoogFilter::process(float input) {
    ...
    // >4.0 is required for self-oscillation to overcome numerical and tanh damping
    float feedback = resonance_ * 5.0f;
...
float LadderFilter::process(float input) {
    ...
    float feedback = resonance_ * 5.0f;  // Increased from 3.5 to allow self-oscillation
