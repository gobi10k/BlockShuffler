...
void onMIDICC(uint8_t ch, uint8_t cc, uint8_t val) {
    switch (cc) {
        ...
        case MIDI_CC::FILTER_CUTOFF:
            synth.setFilterCutoff(20.0f + val * 156.0f);
            break;
        case MIDI_CC::FILTER_RESO:
            synth.setFilterResonance(constrain(val / 127.0f, 0.0f, 1.0f));
            break;
        case MIDI_CC::ATTACK:
            synth.setAmpADSR(val * 0.02f, -1, -1, -1);
        ...
        case MIDI_CC::FILTER_KEY_TRACK:
            synth.setFilterKeyTracking(constrain(val / 127.0f, 0.0f, 1.0f));
            break;
        case MIDI_CC::FILTER_ENV_VEL:
            synth.setFilterEnvVelocity(constrain(val / 127.0f, 0.0f, 1.0f));
            break;
        ...
        case MIDI_CC::OSC_MIX:
            synth.setOscMix(constrain(val / 127.0f, 0.0f, 1.0f));
            break;
        ...
        case MIDI_CC::FM_AMOUNT:
            synth.setFMAmount(constrain(val / 5.0f, 0.0f, 10.0f));
            break;
