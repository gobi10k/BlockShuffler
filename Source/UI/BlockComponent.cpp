...
void BlockComponent::paint(juce::Graphics& g) {
    ...
    if (dragTarget) {
        if (dragTargetIsReorder) {
            g.setColour(juce::Colour(0xFF9966FF).withAlpha(0.9f));
...
void BlockComponent::setDragTargetMode(bool isReorder) {
    if (dragTargetIsReorder == isReorder) return;
    dragTargetIsReorder = isReorder;
    repaint();
}
...
void BlockComponent::mouseDrag(const juce::MouseEvent& e) {
    if (e.getDistanceFromDragStart() > 8 && block) {
