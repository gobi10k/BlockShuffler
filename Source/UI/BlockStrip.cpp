...
void BlockStrip::paintOverChildren(juce::Graphics& g) {
    ...
    if (currentDropAction == DropAction::Stack
        ...
        // Blue vertical insertion line between slots
        ...
        g.drawLine(float(lineX), float(viewport.getY() + 4),
                   float(lineX), float(viewport.getBottom() - 4), 2.5f);
...
void BlockStrip::rebuildBlocks() {
    ...
    for (auto* block : project->blocks) {
        ...
        bc->onDragMoved = [this](BlockComponent* comp, juce::Point<int> centre) {
            updateDragFeedback(comp, centre);
        };
        bc->onDragEnded = [this](BlockComponent* comp, juce::Point<int> centre) {
            blockDropped(comp, centre);
        };
...
void BlockStrip::updateDragFeedback(BlockComponent* draggedComp, juce::Point<int> centre) {
    if (!project) return;
